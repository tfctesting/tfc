...
def client_scheduler(queues:         'QueueDict',
    ...
    while True:
        with ignored(EOFError, KeyboardInterrupt):
            ...
            command, ser_public_keys, is_existing_contact = queues[CONTACT_MGMT_QUEUE].get()  # type: str, bytes, bool
...
def client(onion_pub_key:         bytes,
    ...
    while True:
        with ignored(EOFError, FunctionReturn, KeyboardInterrupt):
            ...
            is_online, check_delay   = manage_contact_status(url_token_public_key_hex, check_delay, is_online, short_addr)

            if not is_online:
                continue

            url_token, cached_pk = update_url_token(url_token_private_key,
                                                    url_token_public_key_hex, cached_pk,
                                                    onion_pub_key, queues)
