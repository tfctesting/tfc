...
from src.common.statics    import (COMMAND_DATAGRAM_HEADER, DATAGRAM_HEADER_LENGTH, DST_COMMAND_QUEUE,
                                   ...
                                   ORIGIN_USER_HEADER, PUBLIC_KEY_DATAGRAM_HEADER,
    ...
    while True:
        with ignored(EOFError, KeyboardInterrupt, SoftError):
            ...
            if header == UNENCRYPTED_DATAGRAM_HEADER:
                ...
                process_file_datagram(ts, packet, header)
                ...
                process_group_management_message(ts, packet, header)
...
def process_message_datagram(ts:     'datetime',
    ...
    buffer_to_flask(packet_str, onion_pub_key, ts, header)
...
def process_file_datagram(ts:     'datetime',
    ...
    for onion_pub_key in pub_keys:
        buffer_to_flask(file_ct, onion_pub_key, ts, header, file=True)
...
def process_group_management_message(ts:                'datetime',
    ...
    if header in [GROUP_MSG_INVITE_HEADER, GROUP_MSG_JOIN_HEADER]:
        ...
        if header == GROUP_MSG_MEMBER_ADD_HEADER:
            ...
            process_add_or_group_remove_member(ts, header, header_str, group_id, before_adding, new_in_group)
            ...
            process_add_or_group_remove_member(ts, header, header_str, group_id, remaining, removable)

    elif header == GROUP_MSG_EXIT_GROUP_HEADER:
        process_group_exit_header(ts, packet, header, header_str, group_id)
