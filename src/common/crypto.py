...
def csprng(key_length: int = SYMMETRIC_KEY_LENGTH) -> bytes:
    ...
    Generator (LRNG) is available[1]. (The pre-4.8 kernel CSPRNG is called
    /dev/random in accordance with the original documentation[2]).
    ...
    The entropy sources of the LRNG include
        o RDRAND CPU instruction:
          ...
          to provide only small amount of entropy (8 bits).[2]

        o CPU Jitter RNG, that is assumed to provide a 1/16th bit of
          entropy per generated bit.[2]

        o Some HWRNGs if their drivers are available.[2]

        o Data written to /dev/(u)random from userspace.[2]

        o User space IOCTL of RNDADDENTROPY.[2]

        o Keyboard presses and mouse movements (ignored by the entropy
          estimator).[2]
    ...
    SHA-1 hash of the entropy pool is mixed back to the pool to provide
    backtracking resistance.[2]
    ...
    also seeded with 256 bits of entropy.[2] TFC uses the secondary DRNG
    ...
    seeded level (256-bits).[2]

    All this means TFC running in the user-space is always using fully
    seeded LRNG.

    DRNG
    ----
    LRNG uses the ChaCha20 stream cipher as the default deterministic
    random number generator (DRNG). The DRNG is seeded by slicing the
    seed into 256-bit chunks. Each chunk is then processed by XORing the
    chunk with the state data that holds the stream cipher key and by
    updating the state. If the last seed chunk is too small, it is still
    XORed with the ChaCha20 state (which is the same as if the seed was
    zero-padded).

    The output of the DRNG is the ChaCha20 keystream (that in the
    context of stream cipher would be XORed with the plaintext to
    produce the ciphertext).

    After the key has been generated, the state update function is
    invoked to ensure backtracking resistance.
    ...
    the largest key this function generates is a 32-byte symmetric key.
     ...
     [1] https://lwn.net/Articles/686033/
     ...
     [5] https://www.python.org/dev/peps/pep-0524/
     [6] https://lwn.net/Articles/606141/
     [7] https://media.ccc.de/v/32c3-7210-pqchacks#video&t=1116
    """
