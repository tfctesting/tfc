...
def csprng(key_length: int = SYMMETRIC_KEY_LENGTH) -> bytes:
    ...
    # LRNG Overview
    """
    The following schematic is modeled after [1; p.19]

                                           [Use in TFC]
                                                 ↑
                                         [BLAKE2b (by TFC)]
                                                 ↑
                  GETRANDOM(GRND_RANDOM)    GETRANDOM()
                       /dev/random,        /dev/urandom
                           ↑                     ↑
                         SHA-1                   |
                           ↑                     |
                   ┏━━━━━━━━━━━━━━━┓      ┏━━━━━━━━━━━━━━━┓
                   ┃ blocking_pool ┃      ┃ ChaCha20 DRNG ┃
                   ┗━━━━━━━━━━━━━━━┛      ┗━━━━━━━━━━━━━━━┛
                                 ↑          ↑
                               SHA-1      SHA-1
                                 |          |
    ...
    The entropy sources of the LRNG include
        ...
        o add_device_randomness: Device driver-related data believed to
          ...
          into the input_pool along with the high-resolution time stamp, 
          XORed with the Jiffies. The value is requested only once, and 
          it is not considered to contain any entropy.
        ...
        o add_hwgenerator_randomness: HWRNGs supported by the Linux
          ...
          input_pool when the entropy estimator's value falls below the 
          set threshold. (The CPU HWRNG is not processed by the function).

        o add_input_randomness: Key presses, mouse movements, mouse
          button presses etc. accompanied with event data from
          add_timer_randomness. Repeated event values (e.g. key presses 
          or small mouse movements) are ignored by the service function. 
          The entropy estimator does not consider HID events to increase 
          entropy of the input_pool.

        o add_disk_randomness: Hardware events of block devices, e.g.
          HDDs (but not e.g. SSDs). When disk event occurs, the block
          device number is mixed into the input_pool accompanied by the
          data from add_timer_randomness.
        ...
        o add_interrupt_randomness: Interrupts (i.e. signals from SW/HW
          ...
          128-bit, per-CPU pool called fast_pool. The content of the 
          fast_pool is mixed into the input_pool once it has data about 
          at least 64 interrupt events, and at least one second has 
          passed since the last interrupt event mix-in.

        o CPU Jitter RNG (assumed to provide a 1/16th bit of entropy per
          generated bit.)[2]

        o Data written to /dev/(u)random from userspace.[2]
        ...
        o RDRAND CPU instruction:
            - Intel: A pair of inverters feeds 512 bits of raw entropy
                     ...
                     re-seeded after every 511th sample of 128 bits (~8kB).[3]
            ...
            - AMD:   A set of 16 ring oscillator chains feeds 512 bits
                     ...
                     repeated thrice to create a 384-bit seed for the
                     AES256-CTR based DRBG. The DRBG is re-seeded at
                     least every 2048 queries of 32-bits (8kB).[4]

          Since the CPU HWRNG is not an auditable source, it is assumed
          to provide only small amount of entropy.[2]
    """
    ...
    --------------------------------
    The LRNG is initialized by mixing following data into the entropy pool:
        1. The current time
        2. Data obtained from CPU HWRNG via RDRAND instruction, if available.
        3. System specific information such as OS name, release, version, and
           a HW identifier.

    Seeding of the input_pool
    -------------------------
    After a hardware event has occurred, the entropy of the event value
    is estimated, and both are mixed into the 4096-bit primary entropy
    pool (input_pool) using a function based on a LFSR which is also the 
    state transition function of the input_pool.

    The seeding of the input_pool is done by collecting-- TODO
    
    Each seed is generated by concatenating the SHA-1 hash of the
    input_pool, as well as noise from CPU (if available), output of the
    Jitter RNG and a 32 LSBs of a 64-bit timestamp into a temporary
    seed buffer, before it is fed into the-- TODO
    
    Seeding levels of the input_pool
    --------------------------------
    **Minimally seeded state**
    ...
    available, and they are intended for in-kernel consumers.
    ...
    **Fully seeded state**    
    ...
    GETRANDOM syscall won't be available until the LRNG is fully seeded.

    Output of the input_pool
    ------------------------
    When requested, the input_pool feeds either the
        * the 1024-bit secondary entropy pool (blocking_pool),
          available via LRNG /dev/random or GETRANDOM(GRND_RANDOM), or
        * the ChaCha20 DRNG,
          available via LRNG /dev/urandom or GETRANDOM syscall.

    When outputting entropy from input_pool to blocking_pool or
    ChaCha20 DRNG, or when outputting from blocking_pool to the user,
    the entropy is compressed with SHA-1. In both cases the value is
    "folded", i.e. the hash is sliced into two 80-bit chunks that are
    XORed together. The resulting 80-bit value is then returned to the 
    invoking function, and the original 160-bit hash is mixed back into 
    the input_pool or blocking_pool to provide backtracking 
    resistance.
        If more than 80-bits of entropy is requested, the
    hash-fold-yield-mix-back operation is repeated until requested
    number of bytes is generated (or in the case of /dev/random, until
    the entropy estimator stupidly thinks the internal entropy has
    somehow depleted).
    ...
    The internal 64-byte state of the ChaCha20 DRNG consists of
        - 16-byte constant b'Expand 32-byte k'
        - 32-byte key
        -  4-byte counter
        - 12-byte nonce
    ...
    DRNG initialization
    -------------------
    The DRNG is initialized by using content of the input_pool
    (considered to have poor entropy at this point) for key, counter,
    and nonce. Each of the three values is XORed with the output from 
    CPU HWRNG obtained via RDRAND instruction (if available, otherwise 
    only the key is XORed, with the timestamp from RDTSCP instruction).
        Additionally, the init_time is set to reseed the DRNG the next
    time it's called.

    DRNG initial seeding
    --------------------
    During initialization time of the kernel, the kernel injects four
    sets of data from fast_pool into the DRNG. Each set contains event
    data and timestamps about 64 interrupt events from
    add_interrupt_randomness. In addition, all content from the
    add_device_randomness source is mixed into the DRNG key state using 
    an LFSR with a period of 255. Once the entropy sources have been 
    mixed in, the DRNG is considered initially seeded (with entropy of
    128 bits), and add_device_randomness starts to feed the input_pool.

    During initialization time of the kernel, once the input_pool 
    has reached 128 bit entropy, the DRNG is reseeded, after which the 
    DRNG is considered fully seeded (with entropy of 256 bits).

    DRNG state transition and output
    --------------------------------
    The DRNG output is generated by repeatedly calling the ChaCha20 block
    operation which produces 64-byte output block, and increments the
    internal counter by one.

    When outputting from ChaCha20 DRNG to the user, the ChaCha20 block
    function is invoked repeatedly until the requested number of bytes
    are generated. Each invoke yields 64 byte output block that is
    ...
    cipher would be XORed with the plaintext to produce the ciphertext.
        With each generated block the internal 32-bit counter of the
    stream cipher is incremented to ensure unique blocks. The state of
    the DRNG is further stirred by XORing the second 32-bit word of the
    nonce with output from RDRAND instruction, if available.
        Once the random data has been generated, the state update
    function is invoked, which then takes a 256-bit block of unused
    keystream and XORs it with the key part of the ChaCha20 state to
    ensure backtracking resistance.

    DRNG re-seeding
    ---------------
    The DRNG is re-seeded by obtaining (up to) 32 bytes of entropy from
    the input_pool. In the order of preference, the entropy is XORed
    with the output of
    ...
    TFC uses `os.getrandom(size, flags=0)` explicitly. This forces use
    of recent enough Python interpreter (3.6.0 or later) and limits
    Linux kernel version to 3.17 (4.8 is required for the LRNG however).
    The flag 0 means urandom will block if the internal state of the
    CSPRNG has less than 128 bits of entropy.
