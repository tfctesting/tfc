...
def process_group_command(user_input:   'UserInput',
    ...
    func_d    = dict(create=group_create, join=group_create, add=group_add_member, rm=group_rm_member)  # type: FuncDict
...
def validate_group_id(input_parameters: List[str],
    ...
    if command_type == 'join':
        ...
        if group_id in group_list.get_list_of_group_ids():
            raise SoftError("Error: Group with matching ID already exists.", head_clear=True)
...
def group_rm_group(group_name:   str,
    ...
    raise SoftError(f"Removed group '{group_name}'.", head=0, delay=1, tail_clear=True, bold=True)
...
def group_rename(new_name:     str,
    ...
    if window.type == WIN_TYPE_CONTACT or window.group is None:
        raise SoftError("Error: Selected window is not a group window.", head_clear=True)
    ...
    raise SoftError(f"Renamed group '{old_name}' to '{new_name}'.", delay=1, tail_clear=True)
