...
from src.transmitter.key_exchanges import (
    export_onion_service_data,
    new_local_key,
    rxp_load_psk,
    verify_fingerprints,
)
from src.transmitter.packet import (
    cancel_packet,
    queue_command,
    queue_message,
    queue_to_nc,
...
def rxp_show_sys_win(
    ...
    try:
        m_print(
            f"<Enter> returns Receiver to {window.name}'s window",
            manual_proceed=True,
            box=True,
        )
...
def exit_tfc(settings: "Settings", queues: "QueueDict", gateway: "Gateway") -> None:
...
def print_help(settings: "Settings") -> None:
    ...
    def help_printer(tuple_list: List[Union[Tuple[str, str, bool]]]) -> None:
        ...
        """
        len_longest_command = (
            max(len(t[0]) for t in tuple_list) + 1
        )  # Add one for spacing
    ...
    common_commands = [
        ...
        (
            "/logging {on,off}(' all')",
            "Change message log setting (for all contacts)",
            True,
        ),
        ...
        (
            "/notify {on,off} (' all')",
            "Change notification settings (for all contacts)",
            True,
        ),
    ...
    group_commands = [
        ("/group create G A₁..Aₙ", "Create group G and add accounts A₁..Aₙ", n_tm),
        (
            "/group join ID G A₁..Aₙ",
            "Join group ID, call it G and add accounts A₁..Aₙ",
            n_tm,
        ),
...
def determine_selector(
    selection: str, contact_list: "ContactList", group_list: "GroupList"
) -> bytes:
...
def change_setting(
    ...
    propagate_setting_effects(
        setting, queues, contact_list, group_list, settings, window
    )


def check_setting_change_conditions(
    setting: str,
    settings: "Settings",
...
def change_setting_value(
    ...
    if setting in relay_settings:
        ...
        relay_command = (
            UNENCRYPTED_DATAGRAM_HEADER + relay_settings[setting] + value.encode()
        )
        queue_to_nc(relay_command, queues[RELAY_PACKET_QUEUE])


def propagate_setting_effects(
    setting: str,
    queues: "QueueDict",
    contact_list: "ContactList",
    group_list: "GroupList",
...
def verify(window: "TxWindow", contact_list: "ContactList") -> None:
    ...
    contact_list.store_contacts()
    m_print(
        f"Marked fingerprints with {window.name} as '{status_hr}'.",
        bold=True,
        tail_clear=True,
        delay=1,
        tail=1,
...
def whois(
    user_input: "UserInput", contact_list: "ContactList", group_list: "GroupList"
) -> None:
    ...
    if selector in contact_list.get_list_of_addresses():
        m_print(
            [
                f"Nick of '{selector}' is ",
                f"{contact_list.get_contact_by_address_or_nick(selector).nick}",
            ],
            bold=True,
        )
        ...
        m_print(
            [
                f"Account of '{selector}' is",
                f"{contact_list.get_contact_by_address_or_nick(selector).onion_address}",
            ],
            bold=True,
        )
    ...
    # Groups
        ...
        m_print(
            [
                f"Group ID of group '{selector}' is",
                f"{b58encode(group_list.get_group(selector).group_id)}",
            ],
            bold=True,
        )
        ...
        m_print(
            [
                f"Name of group with ID '{selector}' is",
                f"{group_list.get_group_by_id(b58decode(selector)).name}",
            ],
            bold=True,
        )
