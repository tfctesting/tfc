    ...
    while True:
        try:
            ...
            process_command_queue(queues, window_list, contact_list, group_list,
                                  key_list, settings, packet_list, master_key, gateway)

            # File window refresh
            window_list.refresh_file_window_check()

            # Cached messages
            process_cached_messages(window_list, contact_list, group_list, key_list,
                                    settings, packet_list, message_log, file_keys, packet_buffer)
        ...
        for k in file_buffer:
            ...
            try:
                ...
                if key_to_remove:
                    file_buffer.pop(k)
                    file_keys.pop(k)
                    raise FunctionReturn("Cached file processing complete.", output=False)
