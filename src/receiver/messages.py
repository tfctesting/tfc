...
def process_message_packet(ts:                 'datetime',          # Timestamp of received message packet
    ...
    if onion_pub_key == LOCAL_PUBKEY:
        raise FunctionReturn("Warning! Received packet masqueraded as a command.",   window=command_window)
    if origin not in [ORIGIN_USER_HEADER, ORIGIN_CONTACT_HEADER]:
        raise FunctionReturn("Error: Received packet had an invalid origin-header.", window=command_window)
...
def process_message(ts:            'datetime',         # Timestamp of received message packet
    ...
    if header == GROUP_MESSAGE_HEADER:
        ...
        raise FunctionReturn(f"Received file decryption key from {nick}", window=window_list.get_command_window())
