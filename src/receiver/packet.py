...
def decrypt_assembly_packet(packet:        bytes,          # Assembly packet ciphertext
    ...
    try:
        ...
        raise SoftError(f"Warning! Received {p_type} {direction} {nick} had an invalid hash ratchet MAC.",
                        window=cmd_win)
    ...
    if offset < 0:
        raise SoftError(f"Warning! Received {p_type} {direction} {nick} had an expired hash ratchet counter.",
                        window=cmd_win)
...
class Packet(object):
    ...
    def new_file_packet(self) -> None:
        ...
        if not self.contact.file_reception:
            self.add_masking_packet_to_log_file()
            raise SoftError(f"Alert! File transmission from {self.contact.nick} but reception is disabled.")
    ...
    def add_packet(self,
        ...
        try:
            ...
            self.add_masking_packet_to_log_file()
            raise SoftError("Error: Received packet had an invalid assembly packet header.")
