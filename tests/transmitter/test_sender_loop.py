...
from src.transmitter.sender_loop import (
    sender_loop,
...
class TestSenderLoop(unittest.TestCase):
    def test_loops(self):
        ...
        settings = Settings(
            traffic_masking=True, tm_static_delay=0.001, tm_random_delay=0.001
        )
        gateway = Gateway()
...
class TestTrafficMaskingLoop(unittest.TestCase):
    def test_loop(self):
        ...
        def queue_delayer():
            ...
            queue_to_nc(
                UNENCRYPTED_DATAGRAM_HEADER + UNENCRYPTED_EXIT_COMMAND,
                queues[RELAY_PACKET_QUEUE],
            )  # 6
            queue_to_nc(
                UNENCRYPTED_DATAGRAM_HEADER + UNENCRYPTED_WIPE_COMMAND,
                queues[RELAY_PACKET_QUEUE],
            )  # 7
            queues[SENDER_MODE_QUEUE].put(settings)
        ...
        self.assertIsInstance(
            traffic_masking_loop(queues, settings, gateway, key_list), Settings
        )
