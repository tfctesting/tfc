...
from src.common.db_masterkey import MasterKey as OriginalMasterKey
...
from src.transmitter.commands import (
    change_master_key,
    change_setting,
    clear_screens,
    exit_tfc,
...
class TestLogCommand(TFCTestCase):
    ...
    @mock.patch(
        "getpass.getpass",
        side_effect=3 * ["test_password"] + ["invalid_password"] + ["test_password"],
    )
    @mock.patch("time.sleep", return_value=None)
    @mock.patch("builtins.input", return_value="Yes")
    def test_successful_export_command(self, *_):
        ...
        packet = split_to_assembly_packets(
            whisper_header + PRIVATE_MESSAGE_HEADER + b"test", MESSAGE
        )[0]
...
class TestChangeMasterKey(TFCTestCase):
    ...
    def test_invalid_response_from_key_db_raises_fr(self, *_):
        ...
        def mock_sender_loop():
            ...
            while self.queues[KEY_MANAGEMENT_QUEUE].empty():
                time.sleep(0.1)
            if (
                self.queues[KEY_MANAGEMENT_QUEUE].get()[0]
                == KDB_M_KEY_CHANGE_HALT_HEADER
            ):
    ...
    def test_transmitter_command_raises_system_exit_if_key_database_returns_invalid_master_key(
        ...
        self.onion_service.database = TFCDatabase(
            self.onion_service.file_name, self.onion_service.master_key
        )
...
class TestChangeSetting(TFCTestCase):
    ...
    def test_individual_settings(self):
        ...
        self.assertIsNone(
            change_setting(UserInput("set serial_baudrate 9600"), *self.args)
        )
        self.assertEqual(self.queues[RELAY_PACKET_QUEUE].qsize(), 2)

        self.assertIsNone(
            change_setting(UserInput("set allow_contact_requests True"), *self.args)
        )
        self.assertEqual(self.queues[RELAY_PACKET_QUEUE].qsize(), 3)

        self.assertIsNone(
            change_setting(UserInput("set traffic_masking True"), *self.args)
        )
        ...
        self.settings.traffic_masking = False
        self.assertIsNone(
            change_setting(UserInput("set max_number_of_group_members 100"), *self.args)
        )
        self.assertTrue(self.group_list.store_groups_called)
        self.group_list.store_groups_called = False

        self.assertIsNone(
            change_setting(UserInput("set max_number_of_groups 100"), *self.args)
        )
        self.assertTrue(self.group_list.store_groups_called)
        self.group_list.store_groups_called = False

        self.assertIsNone(
            change_setting(UserInput("set max_number_of_contacts 100"), *self.args)
        )
        self.assertEqual(self.queues[KEY_MANAGEMENT_QUEUE].qsize(), 1)

        self.assertIsNone(
            change_setting(UserInput("set log_file_masking True"), *self.args)
        )
...
{BOLD_ON}                                     Alice                                      {NORMAL_TEXT}\n""",
            whois,
            UserInput("whois hpcrayuxhrcy2wtpfwgwjibderrvjll6azfr4tqat3eka2m2gbb55bid"),
            *self.args,
        )
