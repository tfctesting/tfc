...
class TestTor(unittest.TestCase):
    ...
    @mock.patch(
        ...
        side_effect=[MagicMock(), OSError, MagicMock()],
    )
    @mock.patch(
    ...
    @mock.patch(
        ...
        return_value=MagicMock(
            get_info=MagicMock(
                side_effect=[
                    'NOTICE BOOTSTRAP PROGRESS=100 TAG=done SUMMARY="Nope"',
                    'NOTICE BOOTSTRAP PROGRESS=100 TAG=done SUMMARY="Done"',
                ]
            )
        ),
...
class TestOnionService(unittest.TestCase):
    ...
    @mock.patch(
        "time.sleep",
        side_effect=[None, None, KeyboardInterrupt, stem.SocketClosed, None],
    )
    @mock.patch(
        "shlex.split",
        return_value=[
