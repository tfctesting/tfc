...
def assembly_packet_creator(
    ...
    payload: Union[
        bytes, str
    ] = None,  # Payload message content (Plaintext message (str), file data (bytes), or command (bytes))
    ...
    message_header: bytes = None,  # Message header (PRIVATE/GROUP_MESSAGE_HEADER, FILE_KEY_HEADER, or tamper byte)
    ...
    whisper_header: bytes = b"\x00",  # Define whisper-header (b'\x00' =  False, b'\x01' = True, b'\x02' = tampered)
    ...
    # --- Packet encryption ---
    encrypt_packet: bool = False,  # When True, encrypts packet into set of datagrams starting with key (32*b'\x01')
    ...
    if packet_type == MESSAGE:
        ...
        if message_header is None:
            if group_id is not None:
                group_msg_id_bytes = (
                    bytes(GROUP_MSG_ID_LENGTH) if group_msg_id is None else group_msg_id
                )
    ...
    # ---
        ...
        file_key_bytes = (
            os.urandom(SYMMETRIC_KEY_LENGTH) if inner_key is None else inner_key
        )
        ...
        size_bytes = (
            int_to_bytes(file_data_size) if packet_size is None else packet_size
        )
    ...
    for packet in packet_list:
        ...
        encrypted_harac = (
            encrypted_harac if not tamper_harac else tamper_last_byte(encrypted_harac)
        )
        encrypted_message = (
            encrypted_message
            if not tamper_message
            else tamper_last_byte(encrypted_message)
        )

        encrypted_packet = (
            onion_pub_key + origin_header + encrypted_harac + encrypted_message
        )
...
def gen_queue_dict() -> Dict[bytes, Queue]:
    ...
    queue_list = set(
        transmitter_queues + receiver_queues + relay_queues + unit_test_queue
    )
    queue_dict = dict()
