...
STATIC_TIMESTAMP = bytes_to_timestamp(TIMESTAMP_BYTES).strftime("%H:%M:%S.%f")[
    :-TIMESTAMP_LENGTH
]
SLEEP_DELAY = 0.02
        ...
        def queue_delayer():
            ...
            for p in [
                (
                    ...
                    M_S_HEADER + bytes(PADDING_LENGTH),
                    False,
                    False,
                    master_key,
                ),
                ...
                (
                    nick_to_pub_key("Alice"),
                    P_N_HEADER + bytes(PADDING_LENGTH),
                    True,
                    True,
                    master_key,
                ),
    ...
    def test_function_allows_control_of_noise_packets_based_on_log_setting_queue(self):
        ...
        def queue_delayer():
            ...
            for _ in range(3):
                queues[LOG_PACKET_QUEUE].put(
                    noise_tuple
                )  # Not logged because logging_state is False
                time.sleep(SLEEP_DELAY)
...
class TestWriteLogEntry(unittest.TestCase):
    ...
    def test_log_entry_is_concatenated(self):
        for i in range(5):
            ...
            self.assertIsNone(
                write_log_entry(
                    assembly_p, nick_to_pub_key("Alice"), self.tfc_log_database
                )
            )
...
""",
    ...
    def test_long_private_message(self, _):
        ...
        write_log_entry(
            M_A_HEADER + bytes(PADDING_LENGTH),
            nick_to_pub_key("Alice"),
            self.tfc_log_database,
        )
        ...
        for p in assembly_packet_creator(
            MESSAGE, "This is a short message", group_id=GROUP_ID_LENGTH * b"1"
        ):
            write_log_entry(p, nick_to_pub_key("Alice"), self.tfc_log_database)
...
"""
    ...
    def test_short_group_message(self, _):
        ...
        for p in assembly_packet_creator(
            MESSAGE, "This is a short message", group_id=self.window.uid
        ):
            write_log_entry(p, nick_to_pub_key("Alice"), self.tfc_log_database)
            write_log_entry(
                p,
...
"""
    ...
    def test_long_group_message(self, _):
        ...
        for p in assembly_packet_creator(
            MESSAGE, "This is a short group message", group_id=GROUP_ID_LENGTH * b"1"
        ):
            write_log_entry(p, nick_to_pub_key("Alice"), self.tfc_log_database)
...
class TestReEncrypt(TFCTestCase):
    def setUp(self):
        ...
        self.settings = Settings()
        self.tmp_file_name = (
            f"{DIR_USER_DATA}{self.settings.software_operation}_logs_temp"
        )
        self.time = STATIC_TIMESTAMP
        self.log_file = f"{DIR_USER_DATA}{self.settings.software_operation}_logs"
    ...
    def test_missing_log_database_raises_fr(self):
        ...
        self.assert_fr(
            ...
            self.new_master_key.master_key,
            self.settings,
        )
...
class TestRemoveLog(TFCTestCase):
    ...
    def test_removal_of_group_logs(self):
        ...
        for p in assembly_packet_creator(
            MESSAGE, short_msg, group_id=group_name_to_group_id("test_group")
        ):
            write_log_entry(p, nick_to_pub_key("Alice"), self.tfc_log_database)
            write_log_entry(p, nick_to_pub_key("Charlie"), self.tfc_log_database)
        ...
        for p in assembly_packet_creator(
            MESSAGE, short_msg, group_id=group_name_to_group_id("different_group")
        ):
            write_log_entry(p, nick_to_pub_key("David"), self.tfc_log_database)
        ...
        for p in assembly_packet_creator(
            MESSAGE, self.msg, group_id=group_name_to_group_id("different_group")
        ):
            write_log_entry(p, nick_to_pub_key("David"), self.tfc_log_database)
    ...
    def test_removal_of_contact_logs(self):
        ...
        self.assert_fr(
            ...
            remove_logs,
            *self.args,
            selector=group_name_to_group_id("searched_group"),
        )
