...
from src.common.statics import (
    ...
    BLAKE2_KEY_LENGTH_MAX,
    BLAKE2_PERSON_LENGTH_MAX,
    BLAKE2_SALT_LENGTH_MAX,
    PADDING_LENGTH,
    SYMMETRIC_KEY_LENGTH,
    TFC_PRIVATE_KEY_LENGTH,
...
class TestBLAKE2b(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        trimmed_data = file_data[
            2:-1
        ]  # Remove empty lines from the start and the end of the read data.
        ...
        for test_vector in test_vectors:
            ...
            message, key, digest = [
                bytes.fromhex(line.split("\t")[1]) for line in test_vector.split("\n")
            ]
...
class TestArgon2KDF(unittest.TestCase):
    ...
    def test_argon2_cffi_using_the_official_command_line_utility(self):
        ...
        for _ in range(self.number_of_tests):
            ...
            password = "".join(
                [sys_rand.choice(ascii_letters + digits) for _ in range(len_password)]
            )
            salt = "".join(
                [sys_rand.choice(ascii_letters + digits) for _ in range(len_salt)]
            )
            ...
            purported_key = argon2.low_level.hash_secret_raw(
...
class TestXChaCha20Poly1305(unittest.TestCase):
    ...
    ietf_ad = bytes.fromhex("50 51 52 53 c0 c1 c2 c3 c4 c5 c6 c7")
    ...
    def test_encrypt_and_sign_with_the_official_test_vectors(self, mock_csprng):
        ...
        self.assertEqual(
            encrypt_and_sign(self.plaintext, self.key, self.ad),
    ...
    def test_auth_and_decrypt_with_the_official_test_vectors(self):
        ...
        self.assertEqual(
            auth_and_decrypt(self.nonce_ct_tag_libsodium, self.key, ad=self.ad),
            self.plaintext,
        )
    ...
    def test_invalid_tag_in_data_from_database_raises_critical_error(self):
        with self.assertRaises(SystemExit):
            auth_and_decrypt(
                ...
                key=bytes(SYMMETRIC_KEY_LENGTH),
