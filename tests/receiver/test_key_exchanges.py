...
class TestProcessLocalKey(TFCTestCase):
    ...
    def test_invalid_decryption_key_raises_soft_error(self, *_: Any) -> None:
        ...
        # Test
        self.assert_se("Error: Incorrect key decryption key.", process_local_key, self.ts, packet, *self.args)
    ...
    def test_successful_local_key_processing_existing_bootstrap(self, *_: Any) -> None:
        # Setup
        self.key_list.keysets = []
    ...
    def test_keyboard_interrupt_raises_soft_error(self, *_: Any) -> None:
        ...
        # Test
        self.assert_se("Local key setup aborted.", process_local_key, self.ts, bytes(SYMMETRIC_KEY_LENGTH), *self.args)
    ...
    def test_old_local_key_packet_raises_soft_error(self, *_: Any) -> None:
        ...
        # Test
        self.assert_se("Added new local key.", process_local_key, self.ts, self.packet, *self.args)
        self.assert_se("Error: Received old local key packet.", process_local_key, self.ts, self.packet, *self.args)
        self.assert_se("Added new local key.", process_local_key, self.ts, new_packet, *self.args)
    ...
    def test_loading_local_key_from_queue(self, *_: Any) -> None:
        # Setup
        self.key_list.keysets = []
        new_key               = os.urandom(SYMMETRIC_KEY_LENGTH)
        new_hek               = os.urandom(SYMMETRIC_KEY_LENGTH)
...
class TestLocalKeyRdy(TFCTestCase):
    ...
    @mock.patch('time.sleep', return_value=None)
    def test_local_key_installed_no_contacts(self, _: Any) -> None:
        # Setup
...
""", local_key_rdy, self.ts, self.window_list, self.contact_list)
    ...
    @mock.patch('time.sleep', return_value=None)
    def test_local_key_installed_existing_contact(self, _: Any) -> None:
        # Setup
        self.window_list                 = WindowList(nicks=[LOCAL_ID, 'Alice'])
        self.contact_list                = ContactList(nicks=[LOCAL_ID, 'Alice'])
        self.window_list.active_win      = self.window_list.get_window(nick_to_pub_key('Alice'))
...
class TestKeyExECDHE(TFCTestCase):
    ...
    @mock.patch('time.sleep', return_value=None)
    def test_add_ecdhe_keys(self, _: Any) -> None:
...
class TestKeyExPSKTx(TFCTestCase):
    ...
    def test_invalid_nick_raises_soft_error(self, _: Any) -> None:
        ...
        self.assert_se("Error: Received invalid contact data", key_ex_psk_tx, *self.args)

    @mock.patch('time.sleep', return_value=None)
    def test_add_psk_tx_keys(self, _: Any) -> None:
...
class TestKeyExPSKRx(TFCTestCase):
    ...
    def test_invalid_psk_data_raises_soft_error(self, _: Any) -> None:
        ...
        # Test
        self.assert_se("Error: The PSK data in the file was invalid.", key_ex_psk_rx, *self.args)
    ...
    @mock.patch('builtins.input',  return_value=file_name)
    def test_invalid_keys_raise_se(self, *_: Any) -> None:
        # Setup
        keyset       = self.key_list.get_keyset(nick_to_pub_key("Alice"))
        ...
        # Test
        self.assert_se("Error: Received invalid keys from contact.", key_ex_psk_rx, *self.args)
    ...
    def test_valid_psk(self, *_: Any) -> None:
        # Setup
        keyset       = self.key_list.get_keyset(nick_to_pub_key("Alice"))
        keyset.rx_mk = bytes(SYMMETRIC_KEY_LENGTH)
        keyset.rx_hk = bytes(SYMMETRIC_KEY_LENGTH)
    ...
    def test_valid_psk_keyboard_interrupt_raises_soft_error(self, *_: Any) -> None:
        ...
        self.assert_se("PSK import aborted.",
                       key_ex_psk_rx, *self.args)
