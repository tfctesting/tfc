...
class TestRxWindow(TFCTestCase):
    ...
    def test_contact_window_creation(self):
        ...
        self.assertEqual(
            window.window_contacts[0].onion_pub_key, nick_to_pub_key("Alice")
        )
    ...
    def test_get_command_handle(self):
        ...
        # Test
        self.assertEqual(
            window.get_handle(self.ts, WIN_UID_COMMAND, ORIGIN_USER_HEADER),
            f"{self.time} -!- ",
        )
    ...
    def test_get_contact_handle(self):
        ...
        # Test
        self.assertEqual(
            window.get_handle(self.ts, nick_to_pub_key("Alice"), ORIGIN_USER_HEADER),
            f"{self.time}    Me: ",
        )
        self.assertEqual(
            window.get_handle(self.ts, nick_to_pub_key("Alice"), ORIGIN_CONTACT_HEADER),
            f"{self.time} Alice: ",
        )
    ...
    def test_get_group_contact_handle(self):
        ...
        self.assertEqual(
            window.get_handle(
                self.ts, nick_to_pub_key("Charlie"), ORIGIN_CONTACT_HEADER
            ),
            f"{self.time} Charlie: ",
        )
    ...
    def test_print_to_inactive_window_preview_off(self, _):
        ...
        self.assert_prints(
            ...
            f"{CURSOR_UP_ONE_LINE}{CLEAR_ENTIRE_LINE}",
            window.print,
            msg_tuple,
        )
...
{3*(CURSOR_UP_ONE_LINE+CLEAR_ENTIRE_LINE)}""",
            window.redraw_file_win,
        )
...
class TestWindowList(TFCTestCase):
    ...
    def test_remove_window(self):
        ...
        self.assertIsNone(
            self.window_list.remove_window(group_name_to_group_id("test_group3"))
        )
        self.assertEqual(len(self.window_list), 2)
        self.assertIsNone(
            self.window_list.remove_window(group_name_to_group_id("test_group2"))
        )
